...
{
    List<Show> sortedAlphabetically = shows.OrderBy(s => s.Name).ToList();
    return sortedAlphabetically.Select(s => s.Name).ToList();
}
...
{
    return shows.Max(n => n.StartYear);
}
...
{
    List<Show> highestRanked = shows.OrderByDescending(s => s.ImdbRating).ToList();
    return highestRanked.Take(3).ToList();
}
...
{
    List<Show> showRanking = shows.OrderByDescending(s => s.ImdbRating).ToList();
    return showRanking.Take(shows.Count - 1).ToList();
}
...
{
    List<Show> comedyShows = shows.Where(s => s.Genres.Contains("Comedy")).OrderBy(s => s.ImdbRating).ToList();
    return comedyShows.Select(s => s.Name).ToList();
}
...
{
    List<Show> multipleGenres = shows.Where(s => s.Genres.Count > 1).OrderBy(s => s.StartYear).ToList();
    return multipleGenres.OrderBy(s => s.Name).ToList();
}
...
{
    List<Show> orderByEnding = shows.OrderBy(s => s.EndYear).Where(s => s.EndYear >= 2000).ToList();
    return orderByEnding.First();
}
...
{
    List<Show> bestDrama = shows.Where(s => s.Genres.Contains("Drama")).OrderByDescending(s => s.ImdbRating).ToList();
    return bestDrama.First();
}

// 18. Return all dramas except for the highest rated.
static List<Show> AllButBestDrama(List<Show> shows)
{
    List<Show> allButBest = shows.Where(s => s.Genres.Contains("Drama")).OrderByDescending(s => s.ImdbRating).ToList();
    return allButBest.Skip(1).ToList();
}

// 19. Return the number of crime shows with an IMDB rating greater than 7.0.
static int GoodCrimeShows(List<Show> shows)
{
    List<Show> goodCrime = shows.Where(s => s.Genres.Contains("Crime") && s.ImdbRating > 7.0).ToList();
    return goodCrime.Count();
}
...
{
    List<Show> firstLongRunningAndTopRated = shows.Where(s => s.EndYear - s.StartYear > 10 && s.ImdbRating < 8).ToList();
    return firstLongRunningAndTopRated.OrderBy(s => s.Name).FirstOrDefault();
}

// 21. Return the show with the most words in the name.
static Show WordieastName(List<Show> shows)
{
    List<Show> longestName = shows.OrderByDescending(s => s.Name.Split(" ").Length).ToList();
    return longestName.First();
}
...
{
    List<string> justNames = shows.Select(s => s.Name).ToList();
    string namesWithCommas = string.Join(", ", justNames);
...
{
    List<string> justNames = shows.Select(s => s.Name).Take(shows.Count - 1).ToList();
    string justBeginningNames = string.Join(", ", justNames);
    List<string> lastName = shows.Select(s => s.Name).ToList();
    string veryLastName = lastName.Last();
    string and = " and ";
    return justBeginningNames + and + veryLastName;
}
